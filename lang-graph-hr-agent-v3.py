...
async def select_skills(state: HRState):
    ...
    prompt = render_prompt(
        "prompts/hr/skill_router.jinja2",
        {
...
async def planner(state: HRState):
    ...
    prompt = render_prompt("prompts/hr/planner.jinja2",
        {
            "user_input": state["user_input"],
            "tool_names": tool_names,
            "tool_schemas": json.dumps(state["tool_schemas"], indent=2),
            "history": json.dumps(state["history"], indent=2)
        }
    )
...
def generate_response(state: HRState):
    ...
    prompt = render_prompt("prompts/hr/response.jinja2",
        {
            "user_input": state["user_input"],
            "history": json.dumps(state["history"], indent=2)
        }
